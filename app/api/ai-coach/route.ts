...
export async function POST(req: NextRequest) {
	try {
		...
		// If OPENAI_API_KEY exists, call OpenAI; otherwise return a helpful fallback response
		const apiKey = process.env.OPENAI_API_KEY;
		if (!apiKey) {
			// Generate more helpful fallback responses based on language
			const fallbackResponses = {
				...
				'km': `ល្អណាស់! ខ្ញុំបានឮអ្នកនិយាយថា: "${text}". ការបញ្ចេញសម្លេងរបស់អ្នកស្តាប់ល្អ! សូមព្យាយាមនិយាយម្តងទៀតដោយមានទំនុកចិត្តច្រើនជាង`
			};
